...
function Leads() {
  ...
  const handleDelete = async (id: number) => {
    if (confirm('Tem certeza que deseja excluir seu lead?')) {
      try {
        ...
        alert(
          'Não foi possivel realizar a operação, favor entrar em contato com o suporte.'
        )
      }
    }
  }
  ...
  useEffect(() => {
    if (createLeadsData?.id) {
      ...
      setCreateMessage({
        msg: 'Não foi possivel realizar a operação, favor entrar em contato com o suporte.',
        type: 'error',
      })
    } else {
      clearMessage()
